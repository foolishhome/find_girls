...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (correspondingIndexPath && [correspondingIndexPath subRow] == 0)
    {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell respondsToSelector:@selector(isExpandable)])
    {
        if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:didSelectRowAtIndexPath:)])
        {
            ...
            {
                if ([_SKSTableViewDelegate respondsToSelector:@selector(tableView:didSelectSubRowAtIndexPath:cell:)])
                    [_SKSTableViewDelegate tableView:self didSelectSubRowAtIndexPath:correspondingIndexPath cell:cell];
            }
        }
    }
    else
...
- (void)expendSelect:(NSIndexPath *)indexPath
{
    UITableViewCell * tc = [self cellForRowAtIndexPath:indexPath];
    if (![tc isKindOfClass:[SKSTableViewCell class]])
        return;
    
    SKSTableViewCell *cell = (SKSTableViewCell *)tc;
    if (![cell respondsToSelector:@selector(isExpandable)])
        return;
