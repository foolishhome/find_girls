...
- (UIView *)expandableView
{
    ...
    button.frame = CGRectMake(0.0, 0.0, self.frame.size.height, self.frame.size.height);

    UIImageView * imgView = [[UIImageView alloc] initWithImage:_image];
    imgView.frame = CGRectMake(0.0, 0.0, _image.size.width, _image.size.height);
    imgView.center = CGPointMake(self.frame.size.height / 2, self.frame.size.height / 2);
...
- (void)addIndicatorView
{
    ...
    CGRect frame = CGRectMake((point.x - CGRectGetWidth(bounds) / 2), point.y * 1.4, CGRectGetWidth(bounds), CGRectGetHeight(self.bounds) - point.y * 1.4);
